...
│                          #   courierSettlement, cashbook (see §10)
...
### 10.1 Order charges — taken at the outcome, not at dispatch

The courier bills by weight, city and what happened, so its charge is only known
when the parcel lands. `utils/orderPosting.js`:

| Step       | Takes                                     | Books                                                                                                                                                                                |
| ---------- | ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| dispatched | courier + **tracking id** (both required) | nothing                                                                                                                                                                              |
| delivered  | **delivery charge** (required, may be 0)  | sale + COGS; COD net of fee + FBR taxes to COD-receivable [courier]. A fee larger than the COD (a **prepaid** order) leaves a _credit_ there — the courier keeps it out of other COD |
| returned   | **return charge** (required, may be 0)    | sale reversed; Dr Return charges / **Cr COD-receivable [courier]** — couriers deduct charges from COD, so cash never moves                                                           |
| exchanged  | **pickup charge** (required, may be 0)    | sale reversed; the original **delivery fee is re-booked** (the courier did deliver); pickup charge as above                                                                          |

### 10.2 Courier lump-sum settlement

A courier remits weekly, one transfer for many orders. `utils/courierSettlement.js`:
one entry (Dr Bank / Cr COD-receivable [courier]), then its delivered unpaid
orders are marked paid from what the courier **still owes afterwards** —
zero-or-negative nets (prepaid parcels) first, then oldest first — so charges it
already kept back are counted and **an order is never part-paid**. Paying the
whole balance pays every order. Reversing the settlement's journal entry
un-pays its orders (`unsettleCourier`), and an order paid this way can't be
marked unpaid on its own.
...
| courier            | refused                                                  | lump-sum settlement (§10.2)                                                 |
